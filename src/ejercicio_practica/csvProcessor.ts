...
export class CSVProcessor extends FileProcessor {
  public parseContent(content: string): Item[] {
    ...
    .filter(line => {
      ...
      return !isNaN(Number(itemNumber)) && !isNaN(Number(weight)) && !isNaN(Number(benefit));
    })
    .map(line => {
      const [itemNumber, weight, benefit] = line.split(',');
      return { itemNumber: Number(itemNumber), weight: Number(weight), benefit: Number(benefit) };
    });
    return items;
  }
